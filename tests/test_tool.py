...
from mythings.testing import FakeGh, ScriptedEngine, make_git_repo

from mydesigner.tool import LEDGER_KIND, TOOL, Result, Tool
...
def test_real_apply_writes_the_mockup_under_mockups(clone: Path, tmp_path: Path) -> None:
    engine = ScriptedEngine(reply="<html><body>hub</body></html>")
    runner = gh_for(issues=[issue_obj(number=9, title="Ecosystem Hub")])
    written: dict[str, str] = {}
    git_calls: list[tuple[Path, list[str]]] = []

    def spy_git(tree: Path, argv: list[str]) -> None:
        git_calls.append((tree, argv))
        if argv[0] == "add":
            written["content"] = (tree / "mockups" / "ecosystem-hub.html").read_text()
    ...
    result = tool.run(issue_number=9)
    assert result.outcome == "success"
    assert [argv[0] for _, argv in git_calls] == ["checkout", "add", "commit", "push"]
    assert written["content"] == "<html><body>hub</body></html>\n"
    (call,) = engine.calls
    assert call.prompt == "Ecosystem Hub\n\ndetails"
...
def test_run_opens_a_draft_pr_when_apply_changes_something(clone: Path, tmp_path: Path) -> None:
    ...
    assert "my-designer/4" in create
